...
class DatabasePelea (context: Context) : SQLiteOpenHelper(context, DATABASE, null, DATABASE_VERSION){
    ...
    companion object {
        ...
        private const val COLUMN_ID_MONSTRUO = "id_mon"
        private const val COLUMN_RESULTADO = "nombre"
    }
    override fun onCreate(db: SQLiteDatabase) {
        val createTable = "CREATE TABLE $TABLA_PELEA ($KEY_ID INTEGER PRIMARY KEY, $COLUMN_UID_PERSONAJE TEXT, $COLUMN_ID_MONSTRUO INTEGER, $COLUMN_RESULTADO TEXT)"
    ...
    fun insertarPelea(uid:String, idMonstruo:Int, res:String):Long  {
        val db = this.writableDatabase
        val values = ContentValues().apply {
            put(COLUMN_UID_PERSONAJE, uid)
            put(COLUMN_ID_MONSTRUO,idMonstruo)
    ...
    fun mostrarPeleas(uid:String){
        ...
        if (cursor != null && cursor.moveToFirst()) {
            do {
                val id_mons = cursor.getInt(cursor.getColumnIndex(COLUMN_ID_MONSTRUO))
                val resul = cursor.getString(cursor.getColumnIndex(COLUMN_RESULTADO))
                val tm=TiposMonstruos()
                println("Has luchado contra el monstruo ${tm.infoMonstruo(id_mons).getNombre()} nivel ${tm.infoMonstruo(id_mons).getNivel()} y has $resul.")
